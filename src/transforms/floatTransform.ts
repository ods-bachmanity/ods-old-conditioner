...
export class FloatTransform extends BaseTransform {
        ...
        const result: Promise<boolean> = new Promise((resolve, reject) => {
            ...
            try {
                ...
                if(!valueText || isNaN(valueText)) {
                    const warnText = `Attempted to transform float but field ${this.fieldName} did not exist or was not numeric.`
                    this.logger.warn(this.correlationId, warnText, `FloatTransform`)
                ...
                this.logger.warn(this.correlationId, ex.message, `FloatTransform`)
    ...
    private validateMinMax(value: any, min: number, max: number): boolean {
        ...
        if (!isNullOrUndefined(min) && !isNullOrUndefined(max)) {
            if (value < min || value > max) {
                this.log(`Attempted to transform float but field ${this.fieldName} was either less than ${min} or greater than ${max}.`)
                return false
            }
            return true
        }
        if (!isNullOrUndefined(min)) {
            if (value < min ) {
                this.log(`Attempted to transform float but field ${this.fieldName} was less than ${min}.`)
                return false
            }
            return true
        }
        if (!isNullOrUndefined(max)) {
            if (value > max) {
                this.log(`Attempted to transform float but field ${this.fieldName} was greater than ${max}.`)
                return false
            }
            return true
        }
    ...
    private log(message: string) {
        ...
        this.logger.warn(this.correlationId, message, `FloatTransform`)
